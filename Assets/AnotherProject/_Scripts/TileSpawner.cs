...
namespace TempleRun
{
    public class TileSpawner : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            for (int i = 0; i < tileStartCount; ++i)
            {
                SpawnTile(startingTile.GetComponent<Tile>(), i != 0); // obstacle on later tiles
            }
        ...
        private void SpawnTile(Tile tile, bool spawnObstacle = false)
        {
            ...
            Quaternion newRotation = Quaternion.LookRotation(currentTileDirection, Vector3.up);
            GameObject newTile = Instantiate(tile.gameObject, currentTileLocation, newRotation);
            prevTile = newTile;
            ...
            if (tile.type == TileType.STRAIGHT)
            {
                // Use the EndPoint transform to update next tile location
                Transform endPoint = newTile.transform.Find("EndPoint");
                if (endPoint != null)
                {
                    currentTileLocation = endPoint.position;
                }
                else
                {
                    Debug.LogWarning("EndPoint not found on tile: " + newTile.name);
                }
            }
            // Turn tiles handled in AddNewDirection
        }

        private void SpawnObstacle()
        {
            if (Random.value > 0.2f || obstacles.Count == 0) return;

            GameObject obstaclePrefab = SelectRandomGameObjectFromList(obstacles);
            if (obstaclePrefab == null) return;

            Quaternion obstacleRotation = Quaternion.LookRotation(currentTileDirection, Vector3.up);
            GameObject obstacle = Instantiate(obstaclePrefab, currentTileLocation, obstacleRotation);
            currentObstacles.Add(obstacle);
        }
        ...
        public void AddNewDirection(Vector3 direction)
        {
            ...
            // Move spawn location using previous tile's EndPoint
            Transform endPoint = prevTile.transform.Find("EndPoint");
            if (endPoint != null)
            {
                currentTileLocation = endPoint.position;
            }

            int straightCount = Random.Range(minimumStraightTiles, maximumStraightTiles);
        ...
        private Vector3 GetTileSize(GameObject tile)
        {
            ...
            return col ? col.bounds.size : Vector3.one * 5; // fallback
        }
    }
}
